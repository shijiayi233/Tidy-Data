...
The ordering of theme_bw() and theme() matters – theme() changes a particular element of the plot’s current “theme”. 
...
ggplot(weather_df,aes(x = date, y = tmax, color = name)) +
  ...
  viridis::scale_color_viridis(
    ...
    discrete = TRUE) + # discrete = TRUE: because the color aesthetic is mapped to a discrete variable. In other cases (for example, when color mapped to prcp) you can omit this argument to get a continuous color gradient. The viridis::scale_fill_viridis() function is appropriate for the fill aesthetic used in histograms, density plots, and elsewhere.
  theme_minimal()+
...
`forcats::fct_relevel`: based on your own preference.  
`forcats::fct_reorder`: based on another variable.
```{r}
weather_df %>%
